...
public func InitializeMap(proplist map)
{
	...
	var w = map.Wdt, h=map.Hgt - offset;
	g_map_width = w;
	
	// Bottom lava lake
	map->Draw("Water", nil, [0,h*4/5 + offset,w,h/5 + offset]);
	
	if (t == 0) DrawBigIslandMap(map);
	if (t == 1) DrawSmallIslandsMap(map);
	
	// Alternate texctures
	var icealt_tex = { Algo=MAPALGO_RndChecker, Wdt=2, Hgt=3, Ratio=8 };
	icealt_tex = { Algo=MAPALGO_Turbulence, Op=icealt_tex };
	icealt_tex = { Algo=MAPALGO_And, Op=[Duplicate("Earth"), icealt_tex]};
	map->Draw("Rock", icealt_tex);
...
func DrawBigIslandMap(proplist map)
{
	...
	// Big
	var island = { Algo=MAPALGO_Polygon, X=[0,w,w*6/8,w*2/8], Y=[h*4/10 + offset,h*4/10 + offset,h*7/10 + offset, h*7/10 + offset] };
	island = { Algo=MAPALGO_Turbulence, Op=island, Amplitude=[0, 8] };
	map->Draw("Earth-earth", island, [w/10,h*13/20 + offset,w*8/10,h*3/20 + offset]); 
	// Make sure one row of inner island is drawn because it's used for player spawns
	map->Draw("Earth-earth", nil, [w*3/10,h*13/20 + offset,w*4/10+1,1 + offset]); 
	// Smaller floating
	var n_islands = 12;
	while(n_islands--)
	{
		var x = w*1/10 + Random(w*8/10);
		var y = h*2/10 + Random(h*3/10 + offset);
		map->Draw("Earth-earth", nil, [x,y,1,1]);
	}
	...
	{
		g_player_spawn_positions[i] = [w*3/10 + i*w*4/10/(plrcnt-1), h*13/20-1 + offset];
	}
	return true;
}
...
func DrawSmallIslandsMap(proplist map)
{
	...
	var w = map.Wdt, h=map.Hgt - offset, x, y, szx, szy;
	// Islands in center of map
	var n_islands = 40;
	while(n_islands--)
	{
		y = h*3/10 + offset + Random(h*5/10 - 3);
		...
		map->Draw("Earth-earth", nil, [x-szx,y,1+2*szx,szy]);
	}
	...
	for (var i = 0; i < spawn_island_count; ++i)
	{
		...
		var y = Max(1, h/10) + Abs(x-w/2) * 3*h/10/w  + offset;
