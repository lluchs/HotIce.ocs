...
func DrawBigIslandMap(proplist map)
{
	...
	map->Draw("^Ice-black", island, [w/10,h*13/20,w*8/10,h*3/20]); 
	// Make sure one row of inner island is drawn because it's used for player spawns
	map->Draw("^Ice-black", nil, [w*3/10,h*13/20,w*4/10+1,1]); 
	// Smaller floating
	var n_islands = 12;
	while(n_islands--)
	{
		...
		map->Draw("^Ice-black", nil, [x,y,1,1]);
	}
...
func DrawSmallIslandsMap(proplist map)
{
	...
	while(n_islands--)
	{
		...
		map->Draw("^Ice-black", nil, [x-szx,y,1+2*szx,szy]);
	}
